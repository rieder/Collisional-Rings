...
class BotsRots(object):
    """
    Resolves collisions between particles by treating them as somewhat
    inelastic collisions. The degree of rebounding is controlled by the
    epsilon_n parameter.
    """
    ...
    def handle_collisions(self, particles, primary_indices, secondary_indices):
        ...
        vshift[:, 0] = (
                1./total_mass
                * (1.0+self.epsilon_n) * normal_velocity * nx
                )
        vshift[:, 1] = (
                1./total_mass
                * (1.0+self.epsilon_n) * normal_velocity * ny
                )
        vshift[:, 2] = (
                1./total_mass
                * (1.0+self.epsilon_n) * normal_velocity * nz
                )
